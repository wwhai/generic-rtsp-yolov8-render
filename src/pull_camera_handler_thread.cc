...
void *pull_camera_handler_thread(void *arg)
{
    ...
    {
        fprintf(stdout, "Error: Could not open Camera stream :(%s).\n", args->input_stream_url);
        pthread_exit(NULL);
    }
    ...
    fprintf(stdout, "Camera handler thread started. Pull stream: %s\n", args->input_stream_url);
    ...
    Context *push_camera_thread_ctx;
    {
        push_camera_thread_ctx = CreateContext();
        ThreadArgs push_camera_thread_args = *args;
        push_camera_thread_args.ctx = push_camera_thread_ctx;
        push_camera_thread_args.input_stream = fmt_ctx->streams[video_stream_index];
        pthread_t push_camera_thread;
        // 复制上下文的参数
        AVCodecParameters *params = avcodec_parameters_alloc();
        if (!params)
        {
        ...
        push_camera_thread_args.input_stream_codecpar = params;
        if (pthread_create(&push_camera_thread, NULL, push_rtmp_handler_thread, (void *)&push_camera_thread_args) != 0)
        {
            fprintf(stdout, "Failed to create Camera thread");
            return NULL;
        }
        pthread_detach(push_camera_thread);
    }
...
END:
    fprintf(stdout, "Camera handler thread stopped\n");
    CancelContext(push_camera_thread_ctx);
    CancelContext(record_mp4_thread_ctx);
    pthread_mutex_destroy(&push_camera_thread_ctx->mtx);
