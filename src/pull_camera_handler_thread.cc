...
void *pull_camera_handler_thread(void *arg)
{
    ...
    {
        fprintf(stderr, "Error: Could not open Camera stream :(%s).\n", args->input_stream_url);
        pthread_exit(NULL);
    }
    ...
    fprintf(stderr, "Camera handler thread started. Pull stream: %s\n", args->input_stream_url);
    ...
    Context *push_camera_thread_ctx;
    {
        ...
        {
            fprintf(stderr, "Failed to create Camera thread");
            return NULL;
        }
...
END:
    fprintf(stderr, "Camera handler thread stopped\n");
